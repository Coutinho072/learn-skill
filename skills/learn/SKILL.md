...
## Source modes (optional base material)

When the learner provides base material (a book PDF, lecture notes, a folder), the session runs in one of two modes, chosen at the goal interview and stored in frontmatter as `source_material:` (path) and `source_mode:`:

- **material-only.** The material is the closed world. The DAG follows the material's own structure and emphasis. Every claim must be traceable to it: fact-checker verifies against the material with chapter/page citations in the verdict, never against the web. Content the material does not support is not taught. A learner question beyond the material is answered with an explicit "fora do material" label, or declined into a follow-up topic.
- **cross-check.** The material is the spine (sequence, examples, emphasis), but every core claim is also verified against independent reliable sources. When the material and external sources diverge, the node shows both openly ("o livro afirma X; a diretriz Y recomenda Z") and the verification callout records the divergence with both citations. Divergences on load-bearing facts become Anki cards.

Mechanics: pass the mode and material path in every `fact-checker` and `deep-research-agent` launch prompt. For a large PDF, read the table of contents plus skimmed chapter openings at PLAN, and only the node-relevant pages at TEACH (the Read tool takes page ranges). In material-only mode, Anki cards cite chapter/page. R1 is unchanged in both modes: only the verification target moves.
...
3. **Goal interview.** One AskUserQuestion call, up to 2 questions: objective, and depth/use context. Vague topic: at most 1 extra sharpening call. If the learner provided base material, ask in the same round which source mode applies (material-only / cross-check; see Source modes) and record `source_material:` and `source_mode:` in frontmatter. Output: a one-sentence goal, scope boundaries, and the macro-topic deck (`Learn::<MacroTópico>`, e.g. `Learn::Cardiologia`, `Learn::Estatística`; the broad area, never the session slug). Store it in session frontmatter as `deck:`.
4. **Create `learn/<slug>/session.md`** from the template, `status: probing`. Background reindex (R6).
5. **Parallel launch (single message).** Draft 5-10 core factual claims of the topic from prior knowledge, then launch in ONE message: (a) `deep-research-agent` with topic, goal, and learner profile (wants: key concepts, canonical teaching sequence, common pitfalls, authoritative sources), and (b) `fact-checker` on the core-claims list. With base material, both prompts carry the material path and mode: in material-only, the sequence comes from the material's structure and verification uses the material alone. Do NOT wait for them.
...
8. **Prefetch N1 + approval (single message).** Draft node N1 (motivação → formalização → exemplo; NOT shown yet). In ONE message: launch `fact-checker` (N1 draft + ledger for delta-skip) and `viz-maker` (bias TOWARD a visual: most nodes deserve one, and complex concepts, flowcharts, processes, and relationships all qualify; inline mermaid needs no subagent; conceptual or flow SVG → viz-maker; anatomical or beyond code-drawn art → viz-maker, which may use AI image generation when `OPENAI_API_KEY` is set; skip only when a figure would add nothing over the text), then AskUserQuestion for plan approval: Começar / Ajustar / Mudar objetivo. An adjustment that changes N1 voids the prefetch (R4): fix the plan, re-prefetch, re-ask. Set `status: teaching`.
