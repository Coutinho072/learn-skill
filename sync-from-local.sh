...
sub("skills/learn/SKILL.md", [
    ...
    ("anatomical → viz-maker with gen_image.py; skip only when a figure would add nothing over the text)",
     "anatomical → viz-maker, which may use AI image generation when `OPENAI_API_KEY` is set; skip only when a figure would add nothing over the text)"),
...
sub("agents/viz-maker.md", [
    ...
    ("An organic or anatomical subject (organs, tissues, physical objects) gets GPT Image via",
     "An organic or anatomical subject (organs, tissues, physical objects): if the `OPENAI_API_KEY` env var is set, generate via"),
