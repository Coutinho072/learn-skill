---
name: viz-maker
description: "Create a self-verified visual for a lesson node (/learn): conceptual SVG diagrams rendered and visually checked before delivery; optional AI illustrations for organic/anatomical subjects when OPENAI_API_KEY is set. Input: what the visual must communicate, the target directory, and a filename slug. Output: the Obsidian embed line for the finished asset."
...
## Methodology
1. **Choose the form**: A conceptual/geometric/quantitative idea gets a hand-written SVG. An organic or anatomical subject (organs, tissues, physical objects): if the `OPENAI_API_KEY` env var is set, generate via `python3 ~/.claude/skills/learn/scripts/gen_image.py --prompt "..." --out <dir>/<slug>.png`; otherwise draw a deliberately schematic SVG (simplified shapes, clear labels, never realism).
...
- Unrecoverable failure (rsvg missing, image API down after the script's own failure, 3 iterations still broken): `VISUAL_FAILED: <reason>` and nothing else.
...
- Never invent anatomical detail: keep anatomical visuals schematic and didactic ("diagrama esquemático didático"), never realistic.
